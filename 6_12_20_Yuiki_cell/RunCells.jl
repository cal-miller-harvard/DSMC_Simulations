...
function runsim(lgap, lstage, T1, T2, pflip, flow)
    # DSMC Parameters
    zmax = 0.35 # m
    rmax = 0.03 # m
    timestep = 2E-7 # s
    ...
    zmaxs = [zend]
    σs = [1.3E-18]#[1E-18, 3.0E-18, 1E-18] # collision cross section (m^2)
    Ms = [57.0]#[191.0, 191.0, 106.0] # mass of molecule (AMU)
    ...
    for (i, omega) in enumerate(omegas)
        for (j, M) in enumerate(Ms)
            ...
            open(fname, "w") do f
                ...
                julia /n/home03/calmiller/DSMC_Simulations/6_12_20_Yuiki_cell/ParticleTracingYuiki.jl -T 2.0 -n %d ./cell.surfs ./DS2FF.DAT --omega %.5f --pflip %.5f -m %.5f -M %.5f --sigma %.5E --zmin %.5f --zmax %.5f --stats ./stats_omega_%.5f_M_%.1f_zmax_%.5f_pflip_%.5f.csv --exitstats ./exitstats_omega_%.5f_M_%.1f_zmax_%.5f_pflip_%.5f.csv --saveall 0""", omega, M, zmaxs[i], pflip, omega, M, zmaxs[i], pflip, n_particles, omega, pflip, m, M, σs[j], zmin, zmaxs[i], omega, M, zmaxs[i], pflip, omega, M, zmaxs[i], pflip))
            end
            run(`sbatch $fname`)
        end
    end
...
runsim(args["l"], args["L"], args["T1"], args["T2"], args["pflip"], args["flow"])
